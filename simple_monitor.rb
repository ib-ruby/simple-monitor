...
module IB
  class Gateway
    ...
    def exclude_client account_id
      @accounts.delete_if{|x| x.account == account_id.to_s}
    end
  end
end
...
module Ibo::Helpers
  def gw logger=nil # returns the gateway-object or creates it and does basic bookkeeping
    if IB::Gateway.current.nil?   # only the first time ...
      ...
      gateway_parameter[ :watchlists ] =  read_config :watchlist,  [:Currencies]

      begin
        ib=IB::Gateway.new(  **gateway_parameter) do |g|
          g.logger.level=Logger::INFO
          g.logger.formatter = proc {|severity, datetime, progname, msg| "#{datetime.strftime("%d.%m.(%X)")}#{"%5s" % severity}->#{msg}\n" }
        end
        ib.update_orders				 # read pending_orders
        excluded_accounts = read_config(:exclude)
        excluded_accounts.keys.each{| a | ib.exclude_client a } if excluded_accounts.present?
        # don't handle excluded Accounts
        ib.clients.each{ |a| set_alias[a]} 
        set_alias[ib.advisor]
        update_account_data
  ...
  def locate_contract con_id, account = nil
    watchlist_entry = -> {gw.active_watchlists.map{|y| y.detect{|c| c.con_id.to_i == con_id.to_i} rescue nil }.compact.first }
  ...
  class ContractX # < R '/contract/(\d+)/select'
    def post account_id
      if gw.check_connection
        ...
        contract = if  gw.active_watchlists.map(&:to_human).include? input.keys.first
                     begin
...
module Ibo::Views
  ...
  def contract_mask
    ...
    table do
      ...
      tr do
        gw.active_watchlists[0..2].each.with_index do |  watchlist_class , i|
          td do 
                    ...
                    select( name: watchlist_class.to_human, size:1 ) do
                      watchlist_class.all.each do |y| 
                        if @the_selected_watchlist.present? && @the_selected_watchlist == y
                          ...
                          option( value: y ){ watchlist_class[y].to_human[ 1..-2 ] rescue "undefined" } 
                        end
                      end # each
                    end   # select
                  end     # td
